...
public class Utils {
    ...
    private static void initializeNotesList(Resources resources) {
        if (notesList == null) {
            ...
            notesList = new ArrayList<Note>(){{
                add(new Note("Fix machine", "You could argue that in agreeing to participate in an HBO documentary called Monica in Black and White I had signed up to be shamed and publicly humiliated yet again.", 4, 0, defaultImage, false, getNextNoteColor(resources)));
                ...
                add(new Note("Send post", "Send email about a new job", 0, 0, defaultImage, false, getNextNoteColor(resources)));
                ...
                add(new Note("Fix machine", "You could argue that in agreeing to participate in an HBO documentary called Monica in Black and White I had signed up to be shamed and publicly humiliated yet again.", 0, 0, defaultImage, false, getNextNoteColor(resources)));
    ...
    public static int getNextNoteColor(Resources resources) {
        int[] androidColors = resources.getIntArray(R.array.android_colors);
    ...
    public static View inflateNote(Fragment fragment, ViewGroup rootContainer, int layoutIDtoInflate, Note note, boolean isMultiplyShowing) {
        ...
        TextView hashCode = noteElement.findViewById(R.id.note_hashcode);
        hashCode.setText("#" + note.getID().toString());
            ...
            for (int i = 0; i < elementTasks.size(); i++) {
                ...
                if (!isMultiplyShowing) {
                    taskCheckbox.setOnCheckedChangeListener((buttonView, isChecked) -> {
                        setFlagStrikeThroughText(buttonView, isChecked);
                    });
                }
